...
for scenario in scenarios
    ...
    for i in ensemble_members
        ...
        files_two = file_head*"$(scenario)/$(second_var)/r$(i)i1p1f1_$(scenario)_$(second_var).nc"
        tmps = ncData(files, "tas")
        twos = using_two ? ncData(files_two, second_var) : nothing
        if using_two
            data1 = use_metrics ? sqrt.(metric) .* tmps.data : tmps.data
            data1 = non_dim ? reshape_data(data1) ./ temp_factor : reshape_data(data1)

            twodata = apply_transform(twos.data)
